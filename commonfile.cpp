...
void segmentdisplay(int x1, int x2, int y1, int y2, IplImage * img)
{
	if((x1>0) && (y1>0))
	{
		cvRectangle(img,cvPoint((x1-1),(y1-1)),cvPoint(x2,y2),cvScalar(0,0,0),1);
	}
	else if((x1==0) && (y1>0))
	{
		cvRectangle(img,cvPoint(x1,(y1-1)),cvPoint(x2,y2),cvScalar(0,0,0),1);
	}
	else if((x1>0) && (y1==0))
	{
		cvRectangle(img,cvPoint((x1-1),y1),cvPoint(x2,y2),cvScalar(0,0,0),1);
	}
	else
	{
		cvRectangle(img,cvPoint(x1,y1),cvPoint(x2,y2),cvScalar(0,0,0),1);
	}
}
...
int BinaryToDecimal(char binary[], int BinaryNum)
{
	...
	for(int a=0 ; a<BinaryNum ; a++)
	{
		if(binary[a]=='1')
		{
			decimal = decimal + (int)pow( 2.0, (BinaryNum-1-a));
		}
	}
	return decimal;
}
	...
	if((x1!=x2)&&(y1!=y2))
	{
		for(int y=y1;y<=y2;y++)
			for(int x=x1;x<=x2;x++)
			{
				...
				ptr[x]=(uchar)g3;//小数部分直接截取，而非四舍五入
			}
	}
	else if((x1==x2)&&(y1==y2))
	{
		...
		ptr[x1]=(uchar)g3;//小数部分直接截取，而非四舍五入
	}
	else if((x1==x2)&&(y1!=y2))
	{
		for(int y=y1;y<=y2;y++)
